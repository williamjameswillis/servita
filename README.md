# servita

Asked to create an API and Frontend test automation solutions for <https://reqres.in/> and <https://www.saucedemo.com/> respectively.

First tackled the API test solution as there was a question over tooling for the Frontend part.

## part 1 - Frontend Automation

See [Frontend_Automation/README.md](Frontend_Automation/README.md)

## part 2 - API Automation

See [API_Automation/README.md](API_Automation/README.md) 